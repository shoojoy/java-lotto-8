...
## 기능 구현 순서
 ...
 ### 출력
 - [x] 발행 결과 출력: N개를 구매했습니다. + 각 티켓 번호(오름차순)
 - [x] 통계 표 포맷 출력
 - [x] 총 수익률 출력: 총 수익률은 Y%입니다. (금액 천 단위 콤마, 퍼센트 한 자리)
 ### 입력/재시도
 - [x] 구매 금액 입력 및 검증(실패 시 [ERROR] 후 해당 단계부터 재입력)
 - [x] 당첨 번호(6개) 입력/파싱/검증(실패 시 재입력)
 - [x] 보너스 번호(1개) 입력/검증(실패 시 재입력)
 ### 흐름 조립
 - [x] 구매 금액 -> 장수 계산 -> 티켓 발행/출력
 - [x] 당첨 번호 + 보너스 입력/검증
 - [x] 모든 티켓 매칭 -> Result 집계 -> 통계/수익률 출력